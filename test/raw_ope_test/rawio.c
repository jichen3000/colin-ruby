...
static int
check_aix_size(fhandle)
	int fhandle;
{
  char	buffer[512] ;
  long	 nread ;
  nread = read( fhandle, buffer, 512) ;	
  if (nread < 0)
		rb_raise(rb_eRawIOError,"Read raw file error!!");
  lseek( fhandle, 0, SEEK_SET ) ;
	/*printf( "content=[%s][%02X %02X %02X %02X %02X %02X %02X %02X]\n",
			buffer,	buffer[0], buffer[1], buffer[2],
			buffer[3], buffer[4], buffer[5], buffer[6], buffer[7]) ;*/
  if (!strncmp( buffer, "AIX LVCB", 3 ))
    return 4096;
  else
    return 0;
}
...
/*	if(firstpos_chr(RSTRING(opt)->ptr, '+')>0)
		...
		rb_raise(rb_eRawIOError,"Not support open mode(%s)!!",RSTRING(opt)->ptr);
	*/	
	//very note, under HP  f->path = RSTRING(fname)->ptr will error
	//f->path = RSTRING(fname)->ptr;
	f->path = strdup(RSTRING(fname)->ptr);
	//f->fhandle = open(RSTRING(fname)->ptr, iFlag , 0644) ;
	f->fhandle = open(f->path, iFlag , 0644) ;
	if (f->fhandle < 0)
	    rb_raise(rb_eRawIOError,"file handle(%ld) already max error!!",f->fhandle);
	f->pos = 0;
	f->seek_first = 0;
	f->mode = iFlag;
	//printf("f->path=%s\n",f->path);
	...
	{	
	    if (check_aix_size(f->fhandle) == 4096)
		{
		  f->seek_first = 4096;
		  lseek( f->fhandle, f->seek_first, SEEK_SET ) ;
		  f->pos = f->seek_first;
		}
	}
	else if( !strncmp( S_name.sysname, "Linux", 5 ) )
		;
	// in hp-64 ,  S_name.sysname is HP-UX
...
//	free(f->buffer);
	//posix_memalign(&buffer,f->block_size, len);
	//memset( buffer, 0, len) ; 
	//posix_memalign(&f->buffer,f->block_size, len);
	//memset( f->buffer, 0, len) ; 
	//posix_memalign(RSTRING(str)->ptr,f->block_size, len);


	//nread = read( f->fhandle, buffer, len) ;
	str = rb_str_new(0, len);
	//rb_str_locktmp(str);
	nread = read( f->fhandle, RSTRING(str)->ptr, len) ;	
	//rb_str_unlocktmp(str);
//	RSTRING(str)->ptr = buffer;
//	RSTRING(str)->ptr = f->buffer;
...
//	long loffset;
	...
	// in hp, type "off_t" % type "int" is error. so delete this lines
	//if (loffset % f->block_size > 0)
  	  //rb_raise(rb_eRawIOError,"Seek block size (%lld) must is %d multiple!!",loffset,f->block_size);

	//printf("f->seek_first=%d\n",f->seek_first);
	if (iwhence == SEEK_SET)
		loffset = loffset + f->seek_first;
	//printf("iwhence=%d\n",iwhence);
...
//	printf("len: %d \n",RSTRING(str)->len);
	...
	nread = write( f->fhandle, RSTRING(str)->ptr , RSTRING(str)->len ) ;
	if( nread < 0 )
