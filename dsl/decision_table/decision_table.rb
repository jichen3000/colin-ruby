...
class DecisionTable
    ...
    def get_consequence_column(obj)
        ...
        # p condition_value_column
        @content[condition_value_column]
    end
    def decide(obj)
        ConditionValueColumn.new(@conditions.map{|condition| condition.test_function.call(obj)})
    end
end

Condition = Struct.new(:description, :test_function)
